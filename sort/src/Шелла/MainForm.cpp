...
void __fastcall TfrmMain::butSortClick(TObject *Sender)
{
        ...
        if(Cols1 <= 15){
       Shella(Data1, Cols1, 1);
        }
        if (Cols1 > 15){
         sort_shell1 M5 (Cols1);
                        M5.shell_sort1(Data1);
                        M5.~sort_shell1( );
...
//----------------------------------------------------------
void __fastcall TfrmMain::Shella(int *X, int n, int DS){
 bool c;
        int e, g, i, j;
        int Iter;
        Iter = 0;
        ...
        if(!Iter){
                ...
                for(int i = 0; i < n; i++){
                        frmMain->sgRet->Cells[i][1] = frmMain->sgArray->Cells[i][1];
                        frmMain->sgRet->Cells[i][0] = IntToStr(i + 1);
                }
        }
        else frmMain->eResult->Text = IntToStr(Iter);
...
//---------------------------------------------------------------------------
void sort_shell1::load_list(int input[ ])
{
    for (int i = 0; i < n; i++)
        X[i] = input[i];
}
void sort_shell1::shell_sort1( int input[ ])
{
    int i,h;
    int temp;
    ...
    for ( ; h > 0; h /=3)
    {
       ...
       frmMain->sgRet->ColCount = n;
        for( int i1 = 0; i1<n; i1++){if(Cols1 <= 15) frmMain->sgData->Cells[i1+1][0] = (i1+1);};
	      for (int i1 = 0; i1 < n; i1++) {
