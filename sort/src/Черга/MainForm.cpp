        ...
        : TForm(Owner)
{
        ...
        for(int i = 0; i < 8; i++){
                ...
                sgQueue->Cells[0][i] = IntToStr(i + 1);
        }

        sgQueue->RowCount = Range;
...
void __fastcall TfrmMain::qPushClick(TObject *Sender)
{
 if(Queuen < Range){
                try{
                        Value = eValue->Text.ToInt();
                        sgQueue->Cells[1][Queuen] = IntToStr(Value);
                        eValue->Text = "";
                        Queuen++;
                }
                catch(EConvertError&){
                ...
                MessageBox(Handle, "Черга заповнена", "Помилка", MB_ICONERROR);
        }        
}
//---------------------------------------------------------------------------

void __fastcall TfrmMain::qPopClick(TObject *Sender)
{
int i;
        if(Queuen){
                try{
                        Value = sgQueue->Cells[1][0].ToInt();
                        for(i = 0; i < Queuen; i++){
                                sgQueue->Cells[1][i] = sgQueue->Cells[1][i + 1];
                        }
                        sgQueue->Cells[1][Queuen - 1] = "";
                ...
                MessageBox(Handle, "Черга порожня", "Увага", MB_ICONINFORMATION);
        }        
}
//---------------------------------------------------------------------------
