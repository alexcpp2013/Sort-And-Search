...
void __fastcall TfrmMain::butSortClick(TObject *Sender)
{
        ...
        if(Cols1 <= 15){
        sort_insertion M3 (Cols1);
                        M3.insertion_sort(Data1);
                        M3.~sort_insertion( );
        }
        if (Cols1 > 15){
         sort_insertion1 M3 (Cols1);
                        M3.insertion_sort1(Data1);
                        M3.~sort_insertion1( );
...
//----------------------------------------------------------
void sort_insertion::load_list(int input[ ])
{
    for (int i = 0; i < n; i++){
        X[i] = input[i];
...
void sort_insertion::insertion_sort( int input[ ])
{
    ...
    char *title;
       ...
       S[0] = X[0];
       frmMain->sgData->ColCount = n + 1;  //
       frmMain->sgData->Cells[1][frmMain->sgData->RowCount - 1] = S[0]; //
         Result++;
       ...
       for (int i = 1; i < n ; i++)
		...
		while (( S[j] > temp ) && ( j >= 0))
		     ...
		     S[j+1] = S[j];
                ...
                frmMain->sgData->Cells[i1 + 1][frmMain->sgData->RowCount - 1] = S[i1];//
...
//---------------------------------------------------------------------------
void sort_insertion1::load_list(int input[ ])
{
    for (int i = 0; i < n; i++){
        X[i] = input[i];
...
void sort_insertion1::insertion_sort1( int input[ ])
{
