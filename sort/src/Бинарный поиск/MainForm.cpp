...
//----------------------------------------------------------------------------
int __fastcall TfrmMain::Beenary(int Value)
{
        int Cols, i;
        //int *X;
    ...
    if ( (Value < Data1[0]) || Value > Data1[Cols-1])
    ...
    while (head < tail)
    {
        ...
        bmiddle = middle - 1;
        amiddle = middle + 1;
        if (Value == Data1[middle] || Value == Data1[bmiddle] || Value == Data1[amiddle])
      

        
        {
            if (Value == Data1[middle]) return(middle);
            if (Value == Data1[bmiddle]) return(bmiddle);
            if (Value == Data1[amiddle]) return(amiddle);
        }
        else if (Value > Data1[middle])
        head = middle + 1;   
        else
...
void __fastcall TfrmMain::butSortClick(TObject *Sender)
{
        ...
        if(Ans){
                if(Ans > -1){
                        frmMain->eResult->Text = (IntToStr(Ans + 1));
