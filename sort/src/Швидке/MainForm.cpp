...
void __fastcall TfrmMain::butSortClick(TObject *Sender)
{
        ...
        sort_quick M1 (Cols1);
        M1.load_list(Data1);
        M1.quick_sort(0,(Cols1-1));
        M1.~sort_quick( );
        }
        if (Cols1 > 15){
        sort_quick1 M1 (Cols1);
        M1.load_list(Data1);
        M1.quick_sort1(0,(Cols1-1));
        M1.~sort_quick1( ); 
        ...
        for (int i1 = 0; i1 < N; i1++) {
                ...
                frmMain->sgRet->Cells[i1][1] = Data2[i1];
...
//----------------------------------------------------------
void sort_quick::load_list(int input[ ])
{
    for (int i = 0; i < n; i++)
	X[i] = input[i];
}
...
void sort_quick::quick_sort( int first, int last)
{          
	int temp;
       frmMain->sgData->ColCount = n + 1;
	...
	while (i < j)
                ...
                j -= 1;
			      ...
			      for(int i1=0;i1<n;i1++){
                    ...
                    frmMain->sgData->Cells[i1 + 1][frmMain->sgData->RowCount - 1 ] = X[i1];
                               ...
                               frmMain->sgData->Cells[0][frmMain->sgData->RowCount - 1] = IntToStr (i+1) + " -> " + IntToStr(j+1);
		...
		if(first!=j){
                for (int i1 = 0; i1 < n; i1++){
                 ...
                 frmMain->sgData->Cells[0][frmMain->sgData->RowCount - 1] = IntToStr (j+1) + " -> " + IntToStr(first+1);
...
//---------------------------------------------------------------------------
void sort_quick1::load_list(int input[ ])
{
    for (int i = 0; i < n; i++)
	X[i] = input[i];
}
...
void sort_quick1::quick_sort1( int first, int last)
{          
	int temp;
	...
	quick_sort1(first, j-1);
	quick_sort1(j+1, last);
