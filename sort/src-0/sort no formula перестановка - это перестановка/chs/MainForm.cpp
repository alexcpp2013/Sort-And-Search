...
void sort_selection::selection_sort( int input[ ])
{
        ...
        // Используя цикл FOR, итеративно находим наименьший элемент в списке
        // и перемещаем его в правильную позицию,
        for ( int j = 0; j < (n-1); j++)
        {
                ...
                //Когда найден элемент, который еще меньше, чем наименьший известный до сих 
                //пор элемент, переставляем их местами.
                int temp;
                temp = X[j];
                X[j] = X[lowest];
                X[lowest] = temp;
    ...
    //ShowMessage(AnsiString("Кількість порівнянь = ") + Compare + ".");
